...
# 基础的卷积模块 dilation：空洞卷积的膨胀率；group：分组卷积的组数 bn：是否使用归一化 bias：是否使用偏置项
...
# 通道注意力模块，用于计算通道注意力权重 reduction_ratio：全连接层的缩减比例
...
class ChannelPool(nn.Module):# 通道池化模块用于提取空间维度的最大值和平均值
    def forward(self, x):
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * scale
# 用上面的这些定义cbam模块 gate_channels：输入通道数；pool_types：池化类型 ；no_spatial：是否禁用空间注意力模块
