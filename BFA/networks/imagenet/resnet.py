...
model_urls = {# 存储了不同ResNet模型的预训练权重的URL
...
# BasicBlock：实现了残差学习的基本单元，适用于ResNet-18和ResNet-34等较浅的网络
...
# Bottleneck：用于更深的网络（如ResNet-50及以上），通过1x1卷积减少参数量，同时保持网络的深度。
...
# 这是ResNet网络的主要类，负责构建整个网络结构，包括输入层、多个残差层和全连接层。
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=200, zero_init_residual=False,
                 ...
                 deep_stem=False, stem_width=32, avg_down=False):# 要改num_classes=200，因为是tiny_imagenet
