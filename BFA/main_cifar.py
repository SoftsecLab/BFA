...
def main(args):
        ...
        train_loader = torch.utils.data.DataLoader(
            datasets.CIFAR10(args.dataset_dir, train=True, download=True,
                        transform=transforms.Compose([
                            transforms.Pad(4),# 填充4像素
                            transforms.RandomCrop(32),# 随机裁剪回32*32
                            transforms.RandomHorizontalFlip(),#水平反转增强
                            transforms.ToTensor(),
                            normlizer])),
    ...
    net = create_net(args)# # 示例：构建带 SE 模块的 ResNet-18
                          # 代码会根据 --attention_type se --attention_param 4 添加 SE 层
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch", type=str, default="resnet18",
                        help="network architecture (default: resnet18)")# 模型架构
    ...
    parser.add_argument("--dataset", type=str, default="cifar10",
                        help="training dataset (default: cifar10)")# 数据集选择
    ...
    parser.add_argument("--batch_size", type=int, default=128,
                        help="batch size for training and validation (default: 128)")# 批量大小
